						...
						<div class="col-md-12">
							...
							<?php foreach(json_decode($row['frm_option']) as $k => $v): 
								...
								<li>
									...
									<span class=""><?php 
									if($row['type'] == 'radio_opt'){
										$ans_temp = isset($ans[$row['id']][$k]) ? count($ans[$row['id']][$k]) : 0;
										$radio_blank['dataPoints'][]=array('label'=>$v, 'y'=>$ans_temp);
									}

									echo isset($ans[$row['id']][$k]) ? count($ans[$row['id']][$k]) : 0 ?>/<?php echo $taken 
									?></span>
...
window.onload = function () {
	...
	var chart = new CanvasJS.Chart("chartContainer", {
		theme: "light2",
		title:{
			text: radio_data.text              
		},
		data: [              
		{
			type: "column",
			dataPoints: radio_data.dataPoints
		}
		]
	});
